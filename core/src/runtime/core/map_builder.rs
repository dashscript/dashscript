...
impl MapBuilder<'_> {
    ...
    pub fn native_fn(&mut self, name: &str, func: NativeFunctionHandler) {
        ...
        let name = self.vm.allocate_string_bytes(bytes);
        let nf = NativeFunction { name: TinyString::new(bytes), func };
        let ptr = self.vm.allocate_with(nf);
    ...
    pub fn string_constant(&mut self, name: &str, value: &str) {
        let allocated = self.vm.allocate_static_str(name);
        let allocated_constant = self.vm.allocate_string_bytes(value.as_bytes());
    ...
    pub fn allocate_with(self) -> ValuePtr<Map> {
        self.vm.allocate_with(self.map)
    }
...
impl ClassBuilder<'_> {
    ...
    pub fn allocate_with(mut self) -> ValuePtr<Map> {
        let prototype = Value::Dict(self.vm.allocate_with(self.prototype));
        self.map.insert(self.vm.constants.prototype, (prototype, true));
        self.vm.allocate_with(self.map)
    }

    pub fn allocate_with_with_prototype(mut self) -> (ValuePtr<Map>, ValuePtr<Map>) {
        let prototype = self.vm.allocate_with(self.prototype);
        self.map.insert(self.vm.constants.prototype, (Value::Dict(prototype), true));
        (self.vm.allocate_with(self.map), prototype)
    }
